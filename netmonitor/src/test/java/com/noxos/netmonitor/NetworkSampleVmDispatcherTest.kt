...
class NetworkSampleVmDispatcherTest {
    ...
    fun `a clean verdict is reported as Clean`() = runBlocking {
        ...
        val verdict = NetworkSampleVmDispatcher.checkSample(fakeContext, FakeVmSessionFactory(transport), byteArrayOf(1, 2, 3))
    ...
    fun `a VM session that throws resolves to Unknown instead of propagating`() = runBlocking {
        val verdict = NetworkSampleVmDispatcher.checkSample(fakeContext, ThrowingVmSessionFactory, byteArrayOf(1, 2, 3))
    ...
    fun `the VM session is always closed`() = runBlocking {
        ...
        NetworkSampleVmDispatcher.checkSample(fakeContext, factory, byteArrayOf(1, 2, 3))
